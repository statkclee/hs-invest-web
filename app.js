...
// 질문 순서가 곧 논리다 — 화성이 가진 투자 생태계(①) → 기업 투자 4유형: 관내 유입·관내 재투자·관외 유출·재환입(②~⑤) → 그 결과인 세금과 양질의 일자리(⑥) → 실행 과제(⑦). 지면 규칙: 답 한 줄 → 근거 → 조작 · 「안(案)」·「추정」·「대리」는 답 옆에 적는다.
...
function plot(id, traces, layout = {}) { setTimeout(() => Plotly.newPlot(id, traces, { margin: { l: 60, r: 20, t: 40, b: 60 }, font: { family: "Apple SD Gothic Neo, Noto Sans KR, sans-serif", size: 12 }, paper_bgcolor: "#fff", plot_bgcolor: "#fff", showlegend: false, ...layout }, { displayModeBar: false, responsive: true })); return `<div id="${id}" class="plot"></div>`; }
...
const pwGrowth = () => { const pw = D.power.trend, y0 = Math.min(...pw.map(r => r.연도)), y1 = Math.max(...pw.map(r => r.연도)), g = c => { const a = pw.find(r => r.시군구 === c && r.연도 === y0)?.TWh, b = pw.find(r => r.시군구 === c && r.연도 === y1)?.TWh; return a && b ? Math.round(100 * (b / a - 1)) : null; }; return { y0, y1, hs: g("화성시"), pt: g("평택시") }; };
function sol4(g) {
...
function sol5(g, po, os) {
  return sol("재환입 — 감지 · 대안 · 잠금 · 승계", [`먼저 만날 상대: 수도권 내 유출 ${os.기업}사 · 본사 관내·공장 관외 ${po.length}사`, `지킬 고용: 65세 이상 대표 ${fmt(D.succ.n65)}사 · ${fmt(D.succ.emp65)}명`, "공시 이후 대응은 늦음 → 공시 전 접촉"], [
    { t: "감지", cls: "hot", who: `수도권 내 ${os.기업}사 · 공장 관외 ${po.length}사 · 후반기 설비투자 증가 ${E.action.capacity.length}사`, do: "담당자 분기 면담 · 국민연금 가입자 비율·공시·채용 공고로 이전 징후 확인", kpi: "공시 전 접촉 비율 · 징후→면담 일수" },
...
const FLOWCOL = { "화성 관내": "#1baf7a", "수도권 내": "#2a78d6", "지방": "#B7791F", "해외": "#9B2C2C" };
...
function flowMap(id, scope) {   // scope: "world" | "kr"
  ...
  for (const g of Object.keys(FLOWCOL)) { const rs = rows.filter(r => r.갈래 === g); if (!rs.length) continue;
    traces.push({ type: "scattergeo", mode: "markers+text", name: g, lon: rs.map(r => r.lon), lat: rs.map(r => r.lat), text: rs.map(r => r.억 >= (scope === "world" ? 300 : 1000) ? `${r.지역} ${fmt(r.억)}억` : ""), textposition: "top right", textfont: { size: 11, color: "#262624" },
  ...
  traces.push({ type: "scattergeo", mode: "markers+text", name: "화성", lon: [o[0]], lat: [o[1]], text: ["화성"], textposition: "bottom center", textfont: { size: 12, color: "#262624" }, marker: { size: 14, symbol: "star", color: "#262624", line: { color: "#fff", width: 1.5 } }, hoverinfo: "text", hovertext: ["화성특례시 — 출발점"], showlegend: false });
  const geo = scope === "world" ? { projection: { type: "natural earth" }, lonaxis: { range: [-135, 155] }, lataxis: { range: [-12, 66] }, showcountries: true, countrycolor: "#d9d7cf", showland: true, landcolor: "#F4F3EE", showocean: true, oceancolor: "#ffffff", showframe: false, coastlinecolor: "#c9c7be", bgcolor: "#fff" }
    : { scope: "asia", resolution: 50, lonaxis: { range: [125.4, 130.2] }, lataxis: { range: [32.9, 38.7] }, showcountries: true, countrycolor: "#c9c7be", showland: true, landcolor: "#F4F3EE", showocean: true, oceancolor: "#ffffff", showframe: false, coastlinecolor: "#c9c7be", bgcolor: "#fff" };
  Plotly.newPlot(id, traces, { geo, margin: { l: 0, r: 0, t: 0, b: 0 }, legend: { orientation: "h", x: 0, y: 0, bgcolor: "rgba(255,255,255,.7)" }, font: { family: "Apple SD Gothic Neo, Noto Sans KR, sans-serif", size: 12 }, paper_bgcolor: "#fff" }, { displayModeBar: false, responsive: true });
}

// ── 질문 열둘 ─────────────────────────────────────────────────────────────────
const flowOf = () => { const g = {}; for (const r of D.outflow) { g[r.갈래] ??= { 건: 0, 억: 0, 기업: new Set() }; g[r.갈래].건++; g[r.갈래].억 += r.억 || 0; g[r.갈래].기업.add(r.corp_name); } return g; };
const Q = [
{ id: "eco", n: "①", t: "투자 생태계", q: "집적 · 인력 · 전력 · 부지 · 조례 — 강점과 제약", render() {
  ...
  return chain(1) + answer([`<b>제조업 시군구 1위 넷</b> — 종사자 ${fmt(tot.T02.값)}명 · 사업체 ${fmt(tot.T01.값)}곳 · 부가가치 ${fmt(tot.T06.값 / 1e6, 1)}조 · 설비자산 ${fmt(tot.T07.값 / 1e6, 1)}조`,
    `<b>유리</b> — 집적 · 인력·본사 기능(입지 요인 ●●●)`, `<b>제약</b> — 전력 ${fmt(D.power.top.find(r => r.시군구 === "화성시").TWh, 1)}TWh 전국 2위, ${p.y0}→${p.y1} +${p.hs}%(평택 +${p.pt}%) · 부지 · 지가`],
    [`KOSIS 광업제조업조사 ${K.Y1}(종사자 10명↑) · 한전 시군구 판매량`, "중요도(●) = KDI·국토연·대한상의 조사 종합 판단(데이터 아님)"]) +
  kpi([[fmt(M.kodata_all), "KoDATA 정상영업 기업"], [fmt(M.map_n), "DART 감사보고서 법인"], [fmt(sum(E.ind, r => r.n_가입자)), `국민연금 가입자 · 제조 · ${E.M1}`], [won(salary(E.med_wage)) + "원", "추정 연봉 중위(월 고지액 환산)"], [fmt(sum(E.ind, r => r.증분_조), 1) + "조", `설비자산 증분 ${K.Y0}→${K.Y1}`]]) +
  ...
  <div class="card"><h3>전력 사용량 추이(TWh)</h3>${plot("p1c", [...new Set(D.power.trend.map(r => r.시군구))].map(c => ({ type: "scatter", mode: "lines+markers", name: c, x: D.power.trend.filter(r => r.시군구 === c).map(r => r.연도), y: D.power.trend.filter(r => r.시군구 === c).map(r => r.TWh), line: { width: c === "화성시" ? 3 : 1.5 } })), { showlegend: true, legend: { orientation: "h" } })}</div></div>
  <h2>산업군별 종사자 · 전국 순위 · 산업 구분</h2><div class="grid"><div class="card">${hbar("p1a", ind.slice(0, 15).map(r => r.산업), ind.slice(0, 15).map(r => r.값_T02), ind.slice(0, 15).map(r => C.PAL.gal[r.갈래] || "#c4c4c4"))}<div class="k">색 = 산업 구분 · 파랑 A 거점 · 초록 B 소재·부품 1위 · 보라 C 확산 후보 · 적 D 부재</div></div>
  <div class="card"><h3>산업군 × 네 자 — KOSIS · KoDATA · 국민연금 · DART</h3>${table(E.ind.map(r => ({ ...r, 연봉: salary(r.n_인당고지_만) })), [{ k: "산업" }, { k: "갈래", nm: "구분" }, { k: "k_종사자", nm: "종사자" }, { k: "전국순위", nm: "순위" }, { k: "자산_조", nm: "설비자산(조)", d: 1 }, { k: "증분_조", nm: "4년 증분(조)", d: 1 }, { k: "기업", nm: "KoDATA 기업" }, { k: "n_가입자", nm: "연금 가입자" }, { k: "연봉", nm: "추정 연봉(만)" }, { k: "n_300", nm: "300명↑" }], { height: "380px" })}</div></div>
  <h2>KDI 요인 여섯 × 화성</h2>${table(C.KDI, autoCols(C.KDI), { search: false })}
  <h2>보고서 12편 결론</h2><ul class="bul">${C.INSIGHTS.map(s => `<li>${s}</li>`).join("")}</ul>`; } },

{ id: "cluster", n: "①b", t: "업종 집적", q: "행정동 입지계수 · 동종 근접도", render() {
  ...
  return chain(2) + answer([`<b>그룹 11</b> — 행정동 집중 ${gb.length}(${gb.map(r => r.산업.split("(")[0]).join(" · ")}) · 세부 업종 ${U.length} 중 집중 ${U.filter(r => r.집중).length} · 뭉침 ${bunch.length}`, `<b>의약품 → 향남</b>(2km 동종 이웃 ${bunch.find(r => r.mid === "C21")?.배율 ?? "–"}배) · <b>종이 → 마도</b> · <b>기계·금속 → 정남·팔탄</b> · <b>전자·정밀·지식서비스 → 동탄</b>`, "그룹마다 자리가 다름 → 유치 표적도 자리별로"],
    [`층 1: 전수 ${fmt(CL.N)}사 · LQ = 동 안 비중 ÷ 시 비중 · 집중 = 집중비(단위 HHI ÷ 전체 HHI) 1.5↑ 또는 상위 3동 45%↑`, `층 2: DART 매핑 ${fmt(CL.n_pts)}사 좌표만 · 뭉침 = 2km 이웃 동종 비율 ÷ 전체 비율 2↑ · 점 15↑ · <b>점 없음 = 그 단위에 DART 매핑 법인 15사 미만</b>(전수 12.8만 사는 좌표 없음)`, "그룹 = 정책 단위(잠정) · 1군 국가첨단전략 = 반도체·전자·정밀 · 전기·이차전지 · 바이오·의약·화학"]) +
  `<div class="tools"><b>그룹</b> <select id="cl-grp">${gOpts}</select> <b>세부 업종</b> <select id="cl-mid"></select> <span class="k">행정동 색 = LQ · 점 = 법인 사업장(개인사업자 제외)</span></div>
  <div class="grid"><div class="card" style="grid-column:1/-1"><div id="p_cl_map" class="plot" style="height:560px"></div></div></div>
  <div class="grid"><div class="card"><h3 id="cl-ttl">행정동별 — 기업 · 종업원 · LQ</h3><div id="cl-tbl"></div></div>
  <div class="card"><h3>집중비(행정동) × 동종 이웃 배율(좌표) · 점 크기 = 기업 수</h3><div id="p_cl_sc" class="plot" style="height:380px"></div><div class="k">오른쪽 위 = 행정동도 쏠리고 이웃도 동종 · 파랑 제조 · 회 비제조 · 점 15개↑ 업종만</div></div></div>
  <h2>그룹 11 — 어디에 얼마나</h2>${table(G, [{ k: "산업", nm: "그룹" }, { k: "n", nm: "기업" }, { k: "emp", nm: "종업원(기재)" }, { k: "집중비", d: 2 }, { k: "top3", nm: "상위 3동 %", d: 1 }, { k: "top_dong", nm: "1위 동" }, { k: "top_share", nm: "1위 %", d: 1 }, { k: "top_lq_dong", nm: "LQ 최고 동" }, { k: "top_lq", nm: "LQ", d: 2 }, { k: "n_pts", nm: "점" }, { k: "배율", nm: "동종 이웃 배율", d: 1 }, { k: "집중", f: v => v ? "집중" : "" }, { k: "뭉침", f: v => v ? "뭉침" : "" }], { search: false })}
  ...
  <h2>세부 업종 ${U.length}개</h2>${table(U.map(r => ({ ...r, 그룹명: CL.groups.find(g => g.id === r.그룹)?.nm })), [{ k: "그룹명", nm: "그룹" }, { k: "산업" }, { k: "n", nm: "기업" }, { k: "emp", nm: "종업원(기재)" }, { k: "집중비", d: 2 }, { k: "top3", nm: "상위 3동 %", d: 1 }, { k: "top_dong", nm: "1위 동" }, { k: "top_share", nm: "1위 %", d: 1 }, { k: "top_lq_dong", nm: "LQ 최고 동" }, { k: "top_lq", nm: "LQ", d: 2 }, { k: "n_pts", nm: "점" }, { k: "nn_same_km", nm: "동종 최근접 km", d: 2 }, { k: "nn_any_km", nm: "아무 최근접 km", d: 2 }, { k: "배율", nm: "동종 이웃 배율", d: 1 }, { k: "집중", f: v => v ? "집중" : "" }, { k: "뭉침", f: v => v ? "뭉침" : "" }], { height: "460px" })}
  <h2>읽는 법</h2><ul class="bul">${CL.rules.map(r => `<li>${r}</li>`).join("")}<li>기업 수 적은 동은 LQ 가 튐(동탄8동 의약품 3.6 = 몇 사 차이) → 기업 수와 같이 봄</li><li>좌표 = DART 매핑 법인만 → 뭉침은 규모 있는 기업 기준 · 종업원 = 기재 기업 합</li></ul>`; },
  async after() {
    ...
    const draw = () => { const mid = $("#cl-mid").value, ind = CL.ind.find(r => r.mid === mid), rows = CL.dong_ind.filter(r => r.mid === mid);
      ...
      const traces = [{ type: "choropleth", geojson: EMD_GJ, featureidkey: "properties.dong", locations: locs, z, text: txt, hoverinfo: "text", colorscale: [[0, "#F4F3EE"], [.5, "#e0b4aa"], [1, "#a63d2f"]], zmin: 0, zmax: Math.max(2, ...z), marker: { line: { color: "#262624", width: .6 } }, colorbar: { title: "LQ", thickness: 10, len: .5 } }];
      if (pts.length) traces.push({ type: "scattergeo", lon: pts.map(f => f.geometry.coordinates[0]), lat: pts.map(f => f.geometry.coordinates[1]), mode: "markers", marker: { size: pts.length > 3000 ? 3 : pts.length > 800 ? 4 : 6, color: "#1D4ED8", opacity: pts.length > 3000 ? .45 : .7, line: { color: "#fff", width: pts.length > 800 ? 0 : .6 } }, text: pts.map(f => `${nmOf[f.properties.mid] || f.properties.mid}<br>${f.properties.dong || ""}`), hoverinfo: "text" });
      ...
      $("#cl-ttl").textContent = `${ind.산업} — ${fmt(ind.n)}사 · 점 ${fmt(pts.length)} · 상위 3동 ${ind.top3}% · 집중비 ${ind.집중비}${ind.배율 ? " · 동종 이웃 배율 " + ind.배율 : ""}`;
    ...
    const I = CL.ind.filter(r => r.level === "업종" && r.배율 != null);
    Plotly.newPlot("p_cl_sc", [{ type: "scatter", mode: "markers+text", x: I.map(r => r.집중비), y: I.map(r => r.배율), text: I.map(r => r.산업), texttemplate: I.map(r => r.집중 || r.뭉침 ? "%{text}" : ""), textposition: "top center", textfont: { size: 10 },
      marker: { size: I.map(r => 6 + Math.sqrt(r.n) / 3), color: I.map(r => r.대분류 === "C" ? "#1D4ED8" : "#9B9A92"), opacity: .8 }, hovertemplate: "%{text}<br>집중비 %{x} · 배율 %{y}<extra></extra>" }],
      { margin: { l: 50, r: 20, t: 20, b: 50 }, xaxis: { title: "집중비(행정동 HHI ÷ 전체)" }, yaxis: { title: "동종 이웃 배율(2km)" }, shapes: [{ type: "line", x0: 1.5, x1: 1.5, y0: 0, y1: 1, yref: "paper", line: { dash: "dot", color: "#9B9A92" } }, { type: "line", x0: 0, x1: 1, xref: "paper", y0: 2, y1: 2, line: { dash: "dot", color: "#9B9A92" } }], font: { family: "Apple SD Gothic Neo, Noto Sans KR, sans-serif", size: 12 }, paper_bgcolor: "#fff", plot_bgcolor: "#fff" }, { displayModeBar: false, responsive: true }); } },
...
{ id: "inflow", n: "②", t: "관내 유입", q: "관외 본사 사업장 · 외자 · 관외 기업", render() {
  ...
  return chain(3) + answer([`<b>1차 후보</b> — 본사 관외·사업장 관내 ${M.n_C}사(DART 확인 ${inD.length}사 · 전사 설비투자 ${fmt(sum(inD, r => r.설비투자_억))}억)`, `<b>외자 표적</b> — 수입 ${fmt(D.trade.총수입, 0)}억달러의 ${fmt(D.trade.장비몫, 0)}% = 반도체 장비 3사(관내) · 협력사`, `최근 2년 협약 ${M.mou_2y}`],
    ["본사 = DART 등록 주소 · 「본사 관외·사업장 관내」 = 추정", "관세청 HS6 2025 · 화성 신고 기준"]) +
  `<div class="grid"><div class="card"><h3>관외 본사 ${M.n_C}사 — 본사 시도</h3>${bar("p2a", hqs.map(h => h[0]), hqs.map(h => h[1]), hqs.map(() => "#eb6834"))}</div>
  <div class="card"><h3>수입 품목군 — 유치 표적 · 국산화 수요(억달러)</h3>${hbar("p2b", tg.map(r => r.군), tg.map(r => Math.round(r.수입억달러 * 10) / 10), tg.map(r => C.PAL.judge[r.판정] || "#c4c4c4"))}<div class="k">색 = 판정 · 보라 유치 · 초록 국산화 · 파랑 국산화·유치 · 적 대체 불가</div></div></div>
  <h2>본사 관외 · 관내 사업장 ${inD.length}사(DART 확인)</h2>${table(inD, [{ k: "기업명" }, { k: "산업" }, { k: "본점" }, { k: "관내사업장" }, { k: "종업원", nm: "종업원(구간)" }, { k: "설비투자_억", nm: "설비투자(억)" }], { height: "360px" })}
...
{ id: "self", n: "③", t: "관내 재투자", q: "관내 기업 증설 · 요건 시산", render() {
  ...
  <div id="sim"></div>
  <div class="grid"><div class="card"><h3>성장 유형 — 투자·고용 동반 증가 기업</h3>${bar("p3a", Object.keys(grow), Object.values(grow), ["#2F7A4F", "#1D4ED8", "#C9A227", "#9B9A92"])}</div>
  <div class="card"><h3>관내 투자 공시 ${inv.length}건</h3>${table(inv, [{ k: "기업명" }, { k: "유형" }, { k: "억" }, { k: "공시일" }, { k: "산업" }], { search: false, height: "300px" })}</div></div>
  <h2>후반기 설비투자 증가 기업 ${E.action.capacity.length}사 — 다음 투자 여력</h2>${table(E.action.capacity, [{ k: "기업명" }, { k: "산업" }, { k: "종업원", nm: "종업원(구간)" }, { k: "고용증감5y", nm: "5년 고용(±50)" }, { k: "설비투자_억", nm: "누적(억)" }, { k: "연평균_억", nm: "연평균(억)" }, { k: "후반배", nm: "후반/전반(배)", d: 1 }], { height: "340px" })}
  ...
  after() {
    const run = () => { const iv = +$("#iv").value, em = +$("#em").value, cap = $("#cap").classList.contains("on"), s = simulate(iv, em, cap);
      ...
      $("#sim").innerHTML = kpi([[fmt(s.n) + "사", "요건 충족"], [fmt(s.fac) + "억", "시설투자 보조 " + P.RATE * 100 + "%"], [fmt(s.job) + "억", "고용 보조 · 초과 1인 70만×36개월"], [fmt(s.tot) + "억", `소요 합계 · 기금 ${fmt(P.FUND)}억의 ${pct(s.tot, P.FUND)}%`], [cap ? `${s.hitF} · ${s.hitE}` : "–", "상한 도달 기업 · 시설 · 고용"]]) +
...
{ id: "outflow", n: "④", t: "관외 유출", q: "수도권 · 지방 · 해외", render() {
  ...
  return chain(5) + answer([`<b>관외·해외</b> ${fmt(out)}억 · ${outN}사 · ${order.reduce((s, k) => s + (g[k]?.건 || 0), 0)}건 — 수도권 내 ${fmt(g["수도권 내"]?.억)} · 해외 ${fmt(g["해외"]?.억)} · 지방 ${fmt(g["지방"]?.억)}`, `<b>관내</b> ${fmt(g["화성 관내"]?.억)}억`, "원인: 용지 · 전력 · 계통 — 유형별 대응 상이"],
    [`DART 투자 공시 · 상장 · 2021~2026 · 투자예정지 기준 ${D.outflow.length}건 · 금액 = 공시 항목명 뒤의 수`, "해외 = 생산기지 전략 · 지방 = 국비 보조금 경쟁"]) +
  ...
  <div class="card"><h3>연도별 — 관내 vs 관외·해외(억)</h3>${plot("p4b", [{ type: "bar", name: "관내", x: ys, y: ys.map(y => Math.round(yr[y].관내)), marker: { color: C.PAL.flow["화성 관내"] } }, { type: "bar", name: "관외·해외", x: ys, y: ys.map(y => Math.round(yr[y].관외)), marker: { color: "#9B2C2C" } }], { barmode: "stack", showlegend: true, legend: { orientation: "h" } })}</div></div></div>
...
{ id: "return", n: "⑤", t: "재환입", q: "회귀 유도 · 고용 보전", render() {
  const g = flowOf(), po = E.action.plant_out, sudo = E.action.outflow.filter(r => r.갈래 === "수도권 내"), os = E.action.outflow_sum.find(r => r.갈래 === "수도권 내");
  return chain(6) + answer([`<b>협상 영역</b> — 수도권 내 ${fmt(os.억)}억 · ${os.건}건 · ${os.기업}사`, `<b>되돌릴 후보</b> — 본사 관내·공장 관외 ${po.length}사`, `<b>지킬 고용</b> — 65세↑ 대표 ${fmt(D.succ.n65)}사 · ${fmt(D.succ.emp65)}명`],
    ["네 축 중 「유출 방지·관내 유도」 · 「본사 관내·공장 관외」 = 국민연금 가입자 비율 추정"]) +
  ...
  <h2>승계 — 대표 연령대별 고용</h2><div class="grid"><div class="card">${bar("p5a", D.succ.age.map(r => r.대), D.succ.age.map(r => r.고용), D.succ.age.map(r => /^[678]/.test(r.대) ? "#a63d2f" : "#9B9A92"))}<div class="k">고용(명) · 대표 나이 확인 ${fmt(D.succ.n_age)}사 · 65세↑ 제조 ${fmt(D.succ.n65_mfg)}사 ${fmt(D.succ.emp65_mfg)}명</div></div><div class="card">${table(C.SUCC_ACTION, autoCols(C.SUCC_ACTION), { search: false })}</div></div>`; } },
...
{ id: "jobs", n: "⑥", t: "세수·고용 효과", q: "고용 · 연봉 · 세금과공과", render() {
  ...
  `<div class="grid"><div class="card"><h3>산업군별 추정 평균 연봉(만원)</h3>${hbar("p7a", tier.map(r => r.산업), tier.map(r => salary(r.n_인당고지_만)), tier.map(r => r.차등.startsWith("1군") ? "#1D4ED8" : r.차등.startsWith("2군") ? "#C9A227" : "#9B9A92"))}<div class="k">파랑 1군 · 금 2군 · 회 3군 · 중위 ${won(medSal)}원 · 월 고지액 ÷ ${NPS_RATE * 100}% × 12</div></div>
  ...
  <h2>300명 이상 사업장 ${big.length}곳 — 국민연금 공개자료 · 법인·기관</h2>
  <div class="grid"><div class="card">${bar("p7b", bt.map(r => r.군), bt.map(r => r.가입자), bt.map(r => r.군 === "1군" ? "#1D4ED8" : r.군 === "2군" ? "#C9A227" : "#9B9A92"))}<div class="k">군별 가입자(명) · 사업장 ${bt.map(r => `${r.군} ${r.사업장}`).join(" · ")}</div></div>
...
{ id: "action", n: "⑦", t: "실행 과제", q: "과제 12 · 담당 · 시한 · 완료 기준", render() {
  ...
  const A = [
    { w: 0, c: 0, t: "기업별 담당자 지정", what: `요건 충족 ${s0.n}사 · 근접 ${M.n_near}사 · 본사 관외(DART) ${inD}사 · 수도권 내 유출 ${os.기업}사 · 공장 관외 ${po}사 — 1기업 1담당`, who: "투자유치 주관 부서(잠정)", out: "담당자 배정표(기업·담당·첫 연락일)", done: "명단 100% 배정 · 첫 연락 30일 안", why: `집행 ${M.exec}억 · 협약 ${M.mou_2y}`, ref: ["jobs"] },
    ...
    { w: 1, c: 1, t: "계통 증설 요청", what: `요건 충족 ${s0.n}사 + 후반기 투자 증가 ${cap}사 증설 계획 → MW 합산 → 한전 요청`, who: "투자유치 주관 + 에너지 담당(잠정)", out: "요청서(기업·MW·시기)", done: "요청 MW · 회신 · 확보 MW", why: "전력 +31%(평택 +212%) · 계통 포화", ref: ["eco"] },
    ...
    { w: 1, c: 1, t: "본사 이전 트랙 설계", what: `본사 관외·사업장 관내 ${M.n_C}사(DART 확인 ${inD}사 우선) — 연구소 요건 ½ · 협약 관문`, who: "투자유치 주관 부서", out: "트랙 안 · 첫 협약 후보 10사", done: "첫 협약 1건", why: "인력·본사 기능(●●●) · 정주 = 본사 기능", ref: ["inflow"] },
    ...
    { w: 2, c: 2, t: "차등 지원 안 예산 검증", what: `일률 5% ${fmt(tot.현행_억)}억 → 차등 ${fmt(tot.차등_억)}억(4년 · 기금 ${pct(tot.차등_억, P.FUND)}%) · 1군 상한 300억`, who: "투자유치 + 예산 부서(잠정)", out: "검증 보고(연차별 소요·재원)", done: "안(案) → 확정안 또는 수정안", why: "1군 = 임금 높은 산업군 · 성과 = 양질의 일자리", ref: ["jobs"] },
    ...
    { w: 3, c: 2, t: "AI 전환 트랙 신설", what: "관내 법인 1만 곳 — 투자 20억↑ 또는 인력 10명↑ · 고용 유지 조건 · 기초 스마트공장", who: "투자유치 + 산업 담당 + 예산(잠정)", out: "트랙 안 · 예산 요구서", done: "다음 예산 반영", why: "AI = 기존 공장 전환(데이터센터 유치 아님)", ref: ["self"] }];
...
{ id: "trust", n: "⑨", t: "기업 데이터", q: "무엇을 모았고 · 무엇이 빠졌나", render() {
  ...
  const fun = [...funK, ...funD], funC = [ "#9B9A92", ...funK.slice(1).map(() => C.PAL.hq["B 관내 분리"]), ...funD.map(() => C.PAL.hq["A 본사=사업장"])];
  ...
  const gaps = [
    ...
    { 빠진것: "국민연금 화성 사업장 중 KoDATA 명단에 없는 곳", 업체수: `${fmt(um.사업장)} 사업장 · 가입자 ${fmt(um.가입자)}명 · 월 고지액 ${fmt(um.월고지_억)}억`, 왜: "KoDATA 는 본사 화성인 기업만 — 본사 관외의 공장·물류센터·병원·대학(기아 AutoLand·쿠팡 동탄 등) · 사업자번호 앞 6자리만 공개", 보완: "가입자 100명↑ 은 「관내 유입」 대상 명단으로 씀(내부 대조용 · 배포판 미포함) · 매출은 국민연금에 없어 고지액으로 규모 대리", 기준: `국민연금 ${E.M1}` },
    ...C.GAPS];
  ...
  return chain(10) + answer([`<b>원천 ${C.SOURCES.length}갈래</b> — KoDATA 명단 ${fmt(M.kodata_all)}사 위에 DART 감사보고서 ${fmt(M.map_n)}사(원문 9,097건) · 투자 공시 144건 · 국민연금 사업장 16,570곳 · KOSIS 광업제조업조사 · 관세청 HS6 5,613부호 · 한전 · 브이월드 · 자치법규 9곳 · KDI·국토연·대한상의 선행조사를 포개어 봤다`,
    `<b>그럼에도 빠진 기업 데이터가 있다</b> — 감사보고서는 외감 법인 ${fmt(M.map_n)}사(${rate(M.map_n, M.kodata_all)})만 · 그중 설비투자 금액은 ${fmt(M.cap_amt)}사 · 개인사업자 92,876사(72%)는 재무가 없다 · 국민연금 화성 사업장 ${fmt(um.사업장)}곳(${fmt(um.가입자)}명)은 본사가 관외라 KoDATA 명단에 없다 · 1~2인 사업장은 어느 원천에도 없다`],
    ["기업 하나를 「말할 수 있는」 깊이는 아래 깔때기 — 큰 기업 쪽으로 치우친 표본이라 투자유치(큰 기업 찾기)엔 쓰고 소상공인 정책엔 쓰지 않는다 · 세수 = 세금과공과(대리) · 연봉 = 국민연금 인당 고지액 ÷ 9.5% × 12(추정) · 「본사 관내·공장 관외」 = 추정 · 우대 지원 방안 = 안(案)"]) +
  ...
  <div class="card"><h3>무엇이 안 보이나 · 어떻게 쓰나</h3><ul class="bul"><li>KoDATA 명단은 <b>본사 소재</b> 기준 — 본사가 서울·수원인 기업의 화성 공장·물류센터·병원·대학은 명단에 없다</li><li>국민연금 화성 사업장 16,017곳(공사현장 제외) 중 12,886곳(80%)은 KoDATA 와 이었고 ${fmt(um.사업장)}곳이 남았다 — 사업자번호가 앞 6자리만 공개라 이름으로 잇는다</li><li>이 ${fmt(um.사업장)}곳이 <b>「관내 유입」의 1차 대상</b>이다(②) — 가입자 100명↑ 명단은 내부 대조용이고 배포판엔 싣지 않는다</li><li>매출·설비투자는 국민연금에 없다 — 가입자수·고지액으로 규모만 가늠하고, 감사보고서(본사 관외 C 갈래 ${fmt(M.n_C)}사)로 설비투자를 본다</li><li>1~2인 법인·소규모 개인사업장은 국민연금도 KoDATA 도 비어 있다 — 메울 원천이 없다</li></ul></div></div>
  <h2>별첨 PDF ${C.PDFS.length}편 — 수치 정본</h2>${table(C.PDFS, [{ k: "구분" }, { k: "제목" }, { k: "파일" }], { search: false, height: "400px" })}
  <h2>배포판이 싣지 않는 것</h2><ul class="bul"><li>정확 종업원수·급여·연구개발비 → 종업원 7구간 대표값(25·75·150·250·400·750·1,500) · 5년 증감 ±50 반올림</li><li>개인사업자 상호·식별자 → 행정동 집계만</li><li>투자 공시 원문·상세주소 → 시군까지 · 3D 점 주소 읍면동까지</li><li>국민연금 미매칭 사업장 명단(가입자 큰 순 60곳) → 내부 대조용 · 여기엔 집계만</li></ul>`; } },
];
