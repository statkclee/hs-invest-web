...
{ id: "cluster", n: "①b", t: "업종 집적", q: "행정동 입지계수 · 동종 근접도", render() {
  ...
  return chain(2) + answer([`<b>그룹 11</b> — 행정동 집중 ${gb.length}(${gb.map(r => r.산업.split("(")[0]).join(" · ")}) · 세부 업종 ${U.length} 중 집중 ${U.filter(r => r.집중).length} · 근접 집적 ${bunch.length}`, `<b>의약품 → 향남</b>(2km 안 동종 근접 ${bunch.find(r => r.mid === "C21")?.배율 ?? "–"}배) · <b>종이 → 마도</b> · <b>기계·금속 → 정남·팔탄</b> · <b>전자·정밀·지식서비스 → 동탄</b>`, "그룹마다 자리가 다름 → 유치 표적도 자리별로"],
    [`층 1: 전수 ${fmt(CL.N)}사 · LQ = 동 안 비중 ÷ 시 비중 · 집중 = 집중비(단위 HHI ÷ 전체 HHI) 1.5↑ 또는 상위 3동 45%↑`, `층 2: 법인 ${fmt(CL.n_pts)}사 좌표(개인사업자 제외 · 기업명 없음) · 근접 집적 = 2km 안 이웃 중 동종 비율 ÷ 전체 비율 2↑ · 점 15↑`, "그룹 = 정책 단위(잠정) · 1군 국가첨단전략 = 반도체·전자·정밀 · 전기·이차전지 · 바이오·의약·화학"]) +
  ...
  <div class="card"><h3>행정동 집중비 × 동종 근접 배율 · 점 크기 = 기업 수</h3><div id="p_cl_sc" class="plot" style="height:380px"></div><div class="k">오른쪽 위 = 행정동 집중 + 동종 근접 · 파랑 제조 · 회 비제조 · 점 15개↑ 업종만</div></div></div>
  ...
  <h2>읽는 법</h2><ul class="bul">${CL.rules.map(r => `<li>${r}</li>`).join("")}<li>기업 수 적은 동은 LQ 가 튐(동탄8동 의약품 3.6 = 몇 사 차이) → 기업 수와 같이 봄</li><li>좌표 = 법인 전수(개인사업자 제외) · 종업원 = 기재 기업 합</li></ul>`; },
  async after() {
    ...
    const draw = () => { const mid = $("#cl-mid").value, ind = CL.ind.find(r => r.mid === mid), rows = CL.dong_ind.filter(r => r.mid === mid);
      ...
      if (pts.length) traces.push({ type: "scattergeo", lon: pts.map(f => f.geometry.coordinates[0]), lat: pts.map(f => f.geometry.coordinates[1]), mode: "markers", marker: { size: pts.length > 3000 ? 3 : pts.length > 800 ? 4 : 6, color: "#1E40AF", opacity: pts.length > 3000 ? .45 : .7, line: { color: "#fff", width: pts.length > 800 ? 0 : .6 } }, text: pts.map(f => `${nmOf[f.properties.mid] || f.properties.mid}<br>${f.properties.dong || ""}`), hoverinfo: "text" });
    ...
    Plotly.newPlot("p_cl_sc", [{ type: "scatter", mode: "markers+text", x: I.map(r => r.집중비), y: I.map(r => r.배율), text: I.map(r => r.산업), texttemplate: I.map(r => r.집중 || r.뭉침 ? "%{text}" : ""), textposition: "top center", textfont: { size: 10 },
      marker: { size: I.map(r => 6 + Math.sqrt(r.n) / 3), color: I.map(r => r.대분류 === "C" ? "#1E40AF" : "#9CA3AF"), opacity: .8 }, hovertemplate: "%{text}<br>집중비 %{x} · 배율 %{y}<extra></extra>" }],
      { margin: { l: 50, r: 20, t: 20, b: 50 }, xaxis: { title: "집중비(행정동 HHI ÷ 전체)" }, yaxis: { title: "동종 근접 배율(2km)" }, shapes: [{ type: "line", x0: 1.5, x1: 1.5, y0: 0, y1: 1, yref: "paper", line: { dash: "dot", color: "#9CA3AF" } }, { type: "line", x0: 0, x1: 1, xref: "paper", y0: 2, y1: 2, line: { dash: "dot", color: "#9CA3AF" } }], font: { family: "Pretendard Variable, Pretendard, Apple SD Gothic Neo, sans-serif", size: 12, color: "#374151" }, paper_bgcolor: "#fff", plot_bgcolor: "#fff" }, { displayModeBar: false, responsive: true }); } },
...
{ id: "action", n: "⑦", t: "실행 과제", q: "과제 12 · 담당 · 시한 · 완료 기준", render() {
  ...
  const A = [
    ...
    { w: 1, c: 1, t: "계통 증설 요청", what: `요건 충족 ${s0.n}사 + 최근 2년 투자가 빨라진 ${cap}사 증설 계획 → MW 합산 → 한전 요청`, who: "투자유치 주관 + 에너지 담당(잠정)", out: "요청서(기업·MW·시기)", done: "요청 MW · 회신 · 확보 MW", why: "전력 +31%(평택 +212%) · 계통 포화", ref: ["eco"] },
